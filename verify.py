...
def aggregate(sents, probs, vocab, ivocab, win_size):
    ...
    return aggregated_sents[:-1], aggregated_probs # 末尾のEOSを取り除いておく
...
def main(gpu, path_model, path_corpus, path_config, path_target, path_program, path_img, win_size, path_word2vec):
    MAX_LENGTH = 70
    ...
    model = utils.load_model(path_model, path_config, vocab)
    model.to_gpu(gpu)

    sents = parse(vocab, path_target)
    probs = inspect(model, model_name, sents)
