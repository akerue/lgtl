...
class CXT_BLSTM(chainer.Chain):
    ...
    def __init__(self, vocab_size, word_dim, state_dim, initialW, EOS_ID):
        ...
        if initialW is not None:
            ...
            initialW = None
        self.vocab_size_in = self.vocab_size + 1 # なんで+1 ?
        self.BOS_ID = self.vocab_size_in - 1

        super(CXT_BLSTM, self).__init__(
            cxt_embed=L.EmbedID(self.vocab_size_in, self.word_dim,
                                ignore_label=-1, initialW=initialW),
            tgt_embed=L.EmbedID(self.vocab_size_in, self.word_dim,
                                ignore_label=-1, initialW=initialW),
            ...
            l1=L.Linear(2 * self.state_dim, self.state_dim),
            l2=L.Linear(self.state_dim, self.vocab_size),
        )
    ...
    def forward(self, xs, ms, train=False):
        ...
        ys = [self.mlp(F.concat([y_fwd, y_bwd], axis=1), train=train) for y_fwd, y_bwd in zip(ys_fwd[:-2], ys_bwd[2:])]
    ...
    def mlp(self, s, train):
        return self.l2(F.relu(self.l1(s)))


    #def embed_words(self, xs, ms, train):
    #    return [self.tgt_embed(x) for x in xs]
